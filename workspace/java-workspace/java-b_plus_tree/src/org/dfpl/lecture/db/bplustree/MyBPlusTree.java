...
public class MyBPlusTree implements NavigableSet<Integer> {
    ...
    private MyBPlusTreeNode getParent(MyBPlusTreeNode myBPlusTreeNode, MyBPlusTreeNode target) {
        if (myBPlusTreeNode.isLeaf() || myBPlusTreeNode.getChildren().contains(target))
            return myBPlusTreeNode;

        for (int i = 0; i < myBPlusTreeNode.getChildren().size(); i++) {
            return getParent(myBPlusTreeNode.getChildren().get(i), target);
        }
    ...
    private void removeHelper(MyBPlusTreeNode myBPlusTreeNode, int key) {
        ...
        if (index < myBPlusTreeNode.getKeyList().size() && myBPlusTreeNode.getKeyList().get(index) == key) { // 찾은 경우
            if (myBPlusTreeNode.isLeaf()) { // 리프 노드이면
                ...
                if (root != myBPlusTreeNode && myBPlusTreeNode.getKeyList().size() < (m+1) / 2) {
                    MyBPlusTreeNode parentNode = getParent(root, myBPlusTreeNode);
            ...
            if (myBPlusTreeNode.getChildren().get(index).getKeyList().size() < (m + 1) / 2) { // 키 갯수가 모자란 경우
                if (index != 0 && myBPlusTreeNode.getChildren().get(index - 1).getKeyList().size() >= (m + 1) / 2) { // 자식 노드를 채움
                    ...
                    myBPlusTreeNode.getKeyList().set(index - 1, sibling.getKeyList().get(sibling.getChildren().size() - 1));

                    if (!child.isLeaf())
                        child.getChildren().addFirst(sibling.getChildren().removeLast());

                    sibling.getKeyList().removeLast();
                    ...
                    myBPlusTreeNode.getKeyList().set(index, sibling.getKeyList().getFirst());
        ...
        if (root == myBPlusTreeNode && myBPlusTreeNode.getKeyList().isEmpty()) {
            if (myBPlusTreeNode.getChildren().isEmpty())
                root = null;
            else
                root = myBPlusTreeNode.getChildren().getFirst();
        }
    }
