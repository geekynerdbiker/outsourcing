...
def count(image):
    ...
    corners = cv2.goodFeaturesToTrack(cv2.GaussianBlur(gray, (3, 3), 0), maxCorners=0, qualityLevel=0.3,
                                      minDistance=10)
    circles = cv2.HoughCircles(cv2.blur(gray, (3, 3)), cv2.HOUGH_GRADIENT, dp=1, minDist=50, param1=50, param2=30,
                               minRadius=10,
                               maxRadius=50)
    ...
    if circles is not None:
        ...
        for circle in circles[0, :]:
            x, y, radius = circle
            if image[y, x][0] > np.mean(colors, axis=0):
                cv2.circle(image, (x, y), radius, (0, 0, 255), 2, cv2.LINE_AA)
                bright += 1
            else:
...
# TEST
count(refacor(cv2.imread('1.jpeg')))
