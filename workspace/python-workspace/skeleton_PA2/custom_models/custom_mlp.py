...
class CustomMLP:
    ...
    def backward(self):
        ...
        """
        ############################################## EDIT ################################################
        lossGrad = self.output - self.label

        for i in reversed(range(len(self.layers))):
            layer = self.layers[i]
            input = self.layers[i].input

            layer.grad_w = np.dot(lossGrad, input.T)
            layer.grad_b = np.sum(lossGrad, axis=1, keepdims=True)

            if i > 0:
                reluGrad = np.where(input > 0, 1, 0)
                lossGrad = np.dot(layer.weights.T, lossGrad) * reluGrad
        ####################################################################################################
