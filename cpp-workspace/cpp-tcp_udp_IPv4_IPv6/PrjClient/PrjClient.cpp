...
LRESULT CALLBACK ChildWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEMOVE:
		if (bDrawing && g_bCommStarted) {
			...
			if (g_isUDP) {
				if 
					(g_isIPv6) sendto(g_sock, (char*)&g_drawlinemsg, SIZE_TOT, 0, (struct sockaddr*)&serveraddr6, sizeof(serveraddr6));
				else 
					sendto(g_sock, (char*)&g_drawlinemsg, SIZE_TOT, 0, (struct sockaddr*)&serveraddr4, sizeof(serveraddr4));
			} else 
