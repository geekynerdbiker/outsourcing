...
struct msg message;
{
    ...
    sent.seqnum = next;
    sent.acknum = 0;
    sent.checksum = sent.acknum + sent.seqnum;
    strcpy(sent.payload, message.data);
    
    int i;
    for (i = 0; i < DATA; i++)
        sent.checksum += sent.payload[i];
...
struct pkt packet;
{
    ...
    if (acket.acknum == -1 || packte.seqnum < base || checksum != packet.checksum)
        return -1;
...
struct pkt packet;
{
    ...
    if (packet.seqnum == last + 1)
        inorder = 1;
    
    if (inorder)
        sent.seqnum = packet.seqnum;
    else
        sent.seqnum = last;
