/* Programming Assignment 4: Exercise D
 *
 ...
 * road, but as they enter from opposing directions, they will eventually
 ...
 * WHAT TO TURN IN
 *
 * You must turn in two files: mycode4.c and pa4d.c. mycode4.c should
 * contain you implementation of semaphores, and pa4d.c should contain
...
void Main ()
{
     ...
     * changes to driveRoad.  When your solution is tested, we
...
void InitRoad ()
{
    ...
    shm.printfsync = Seminit(1);
    shm.turn = Seminit(1);
    shm.cars = Seminit(1);
    
    for (i = 0; i < NUMPOS; i++) {
        shm.blocked[i] = Seminit(1);
    }
    
    shm.west.gate = Seminit(1);
    shm.east.gate = Seminit(1);
    
    shm.west.line = Seminit(1);
    shm.east.line = Seminit(1);
}
#define TRUE 1
#define FALSE 0
...
// mph: the speed of the car
{
    ...
    Wait(ISWEST(from) ? shm.west.line : shm.east.line);
    Wait(shm.turn);
    Wait(ISWEST(from) ? shm.west.gate : shm.east.gate);
    Wait(shm.blocked[EDGEPOS(from)]);
    
    EnterRoad (from);
    Wait(shm.cars);
    if (!shm.onroads) (Wait(ISWEST(from) ? shm.east.gate : shm.west.gate));
    Signal(shm.cars);
    ...
    Signal(ISWEST(from) ? shm.west.gate : shm.east.gate);
    Signal(shm.turn);
    
    Wait(shm.printfsync);
    PrintRoad ();
    Printf ("Car %d enters at %d at %d mph\n", c, IPOS(from), mph);
    Signal(shm.printfsync);
    ...
    for (i = 1; i < NUMPOS; i++) {
        ...
        Wait(shm.blocked[np - 1]);
        ProceedRoad ();
        Signal(shm.blocked[p - 1]);
        
        if (EDGEPOS(from) == (p - 1))
            ISWEST(from) ? Signal(shm.west.line) : Signal(shm.east.line);
        Wait(shm.printfsync);
        PrintRoad ();
    ...
    Signal(shm.blocked[EDGEPOS(NEGPOS(from))]);
    ...
    Wait(shm.cars);
    if (!shm.onroads) (Signal(ISWEST(from) ? shm.east.gate : shm.west.gate));
    Signal(shm.cars);
