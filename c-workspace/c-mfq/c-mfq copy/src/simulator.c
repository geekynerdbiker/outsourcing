...
static t_proc *preemtion(int burst_t) {
    t_proc *rst = NULL;
    ...
    for (int i = 0; i < proc_num; i++) {
        if (ready_queue2[i] != NULL) {
            int t = get_burst_t(ready_queue2[i]);
            if (t < min_t) {
                index = i;
                rst = ready_queue2[i];
                min_t = t;
...
int start_sim(void) {
    ...
    while (1) {
        ...
        if (curr_proc == NULL) {
            ...
            if (curr_queue == 2) {
                printf("2\n");
                burst_t = get_burst_t(curr_proc);
                preemtion_proc = preemtion(burst_t);
                if (preemtion_proc != NULL) {
                    // 3-2-2. Push the previous process if the preemtion has occurred
                    curr_proc->queue = 3;
                    if (push_queue(curr_proc) == -1)
                        return -1;
                    curr_proc = preemtion_proc;
                    curr_proc_id = curr_proc->PID;
                    curr_queue = 2;
                }
            }
