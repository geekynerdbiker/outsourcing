...
void initPlayer(Player* player, const char* name) {
    ...
    strcpy(player->equippedItem.name, "");
...
void equipItem(Player* player, Item item) {
    if (strcmp(player->equippedItem.name, "") != 0) {
        printf("이미 다른 아이템을 장착중입니다!\n\n");
        return;
    }
    // 장비 장착
    printf("아이템 '%s'을(를) 장착했습니다. ", item.name);
    strcpy(player->equippedItem.name, item.name);
    player->equippedItem.bonusDamage = item.bonusDamage;
    player->equippedItem.bonusDefense = item.bonusDefense;
