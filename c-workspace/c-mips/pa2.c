...
static int process_instruction(unsigned int instr)
{
	...
	if (check == 0x8000)  
		con = con | 0xffff0000;
	if (form == 0x08) 
		registers[rt] = registers[rs] + con;
	if (form == 0x0c)  
		registers[rt] = registers[rs] & (instr & 0b00000000000000001111111111111111);
	if (form == 0x0d) 
		registers[rt] = registers[rs] | (instr & 0b00000000000000001111111111111111);
	if (form == 0x23) 
		registers[rt] = ((memory[registers[rs] + con]) << 24) | (memory[(registers[rs] + con)+1]) << 16 | (memory[(registers[rs] + con) + 2]) << 8 | (memory[(registers[rs] + con) + 3]);
	if (form == 0x2b) {
		...
		registers[rt] = (registers[rs] < con) ? 1 : 0;
	if (form == 0x04) 
