...
def AllPossible(board, i, j):
    ...
    if p == 'p':
        ...
        for pos in poss:  # in promotion position set weight same as queen
            if pos[0] == 0 and 'w' == board[i][j][0][0]:
                board[i][j][1] = 100
            if pos[0] == 7 and 'b' == board[i][j][0][0]:
                board[i][j][1] = 100
    if p == 'r':
        poss = RookMove(board, i, j)
    if p == 'n':
...
def PickMove(moves):
    ...
    return best if best[3] > best2[3] else best2


# %%
def SortMoves(moves, order='desc'):
    NMoves = len(moves)
...
# Generate Interface Communication function.
def SelectedMove(board, color, listOfPlays=None):
    Promotion(board)
    tempboard = copy.deepcopy(board)  # copy board

    if color == 'w':  # in case of white
        pick1 = SortMoves(ScoreMoves(tempboard, 'w'))  # get sorted available moves
        weights = []  # store best opponent next move weights

        for i in range(len(pick1)):  # append opponent's best move weights on each turn's move
            Move(tempboard, pick1[i][1], pick1[i][2])
            pick2 = SortMoves(ScoreMoves(tempboard, 'b'))[0]
            weights.append((i, pick2[3]))
        weights.sort(key=lambda x: x[1])  # sort in ASC

        return pick1[weights[0][0]]  # return move that causing lowest best weight of opponent's move

    if color == 'b':
        pick1 = SortMoves(ScoreMoves(tempboard, 'b'))
        weights = []

        for i in range(len(pick1)):
            Move(tempboard, pick1[i][1], pick1[i][2])
            pick2 = SortMoves(ScoreMoves(tempboard, 'w'))[0]
            weights.append((i, pick2[3]))
        weights.sort(key=lambda x: x[1])

        return pick1[weights[0][0]]


# Get promotion pawn to queen
def Promotion(board):
    for i in range(8):
        if 'wp' in board[0][i][0][0]:
            board[0][i][0][0] = 'wq'
        if 'bp' in board[7][i][0][0]:
            board[7][i][0][0] = 'bq'


def GameOver(board):
    bkAlive, wkAlive = False, False
    for i in range(8):
        for j in range(8):
            if board[i][j][0] == 'bk':
                bkAlive = True
            ...
            if bkAlive and wkAlive:
                break

    if not bkAlive or not wkAlive:
        return True
    return False


# Test for print
# REMOVE THIS BEFORE SUBMIT PLEASE

# FROM HERE
def Simualtion():
    board = InitChess(PieceValues())
    idx = 0

    while True:
        pick = SelectedMove(board, 'b')
        Move(board, pick[1], pick[2])

        print('[ Move ' + str(idx) + ': Black ]')
        PrintBoard(board)
        print()

        pick = SelectedMove(board, 'w')
        Move(board, pick[1], pick[2])

        if GameOver(board):
            break

        print('[ Move ' + str(idx) + ': White ]')
        PrintBoard(board)
        print()
