...
class TSP:
    ...
    def make_random_path(self):
        ...
        # 모든 도시를 경유하는 path 생성
        # Edit this (1)
        path_edit = list(range(len(self.helper.nodes)))
    ...
    def make_greedy_path(self):
        ...
        while len(pool) > 0:
            ...
            for pool_i in range(len(pool)):
                ...
                # Edit this (1)
                node1 = path_edit[len(path_edit) - 1]
    ...
    def make_2opt_path(self):
        ...
        for trial in range(iteration_num):
            ...
            # 경로를 슬라이싱하기 위한 범위 지정
            # 슬라이드의 설명 참조
            i = int(len(path_edit) * 0.3)  # Edit this (2)
            k = int(len(path_edit) * 0.6)  # Edit this (3)

            # 현재의 경로 path_edit를 3 조각으로 slicing (path1, path2, path3)
            path1 = path_edit[:i]  # Edit this (4)
            path2 = path_edit[i:k]  # Edit this (5)
            path3 = path_edit[k:]  # Edit this (6)
            ...
            if self.helper.get_path_length(path_new) < self.helper.get_path_length(path_edit):  # Edit this (7)
    ...
    # -------------------------------------------------------------------------------------------
    # - 어떤 방법인지에 대한 설명: 2opt에서 착안하여 3opt를 시도함
    #
    #
    # - 2opt와의 성능상 차이에 대한 분석: 뛰어난 성능 차이는 없으나 2opt와 비교했을 때 대체적으로 조금 더 발전한 성능을 보임
    #
    #
    # - 시행착오를 통해 배운점: 2opt에 반해 속도는 더 떨어지나 더 나은 성능을 보이지만, 성능과 속도를 둘 다 고려하여 적절한
    #                     알고리즘의 선택의 필요성을 느낌
    #
    #
    def make_yourown_path(self):
        ...
        for trial in range(iteration_num):
            ...
            # 경로를 슬라이싱하기 위한 범위 지정
            # 슬라이드의 설명 참조
            i1 = int(len(path_edit) * 0.16)
            i2 = int(len(path_edit) * 0.33)
            i3 = int(len(path_edit) * 0.49)
            i4 = int(len(path_edit) * 0.66)
            i5 = int(len(path_edit) * 0.82)

            # 현재의 경로 path_edit를 6 조각으로 slicing
            path1 = path_edit[:i1]
            path2 = path_edit[i1:i2]
            path3 = path_edit[i2:i3]
            path4 = path_edit[i3:i4]
            path5 = path_edit[i4:i5]
            path6 = path_edit[i5:]

            # path2, path4, path6을 각각 거꾸로 하여 다시 이어붙임
            path_new1 = path1 + list(reversed(path2)) + path3 + path4 + path5 + path6
            path_new2 = path1 + path2 + path3 + list(reversed(path4)) + path5 + path6
            path_new3 = path1 + path2 + path3 + path4 + path5 + list(reversed(path6))
            path_new4 = path1 + list(reversed(path2)) + path3 + list(reversed(path4)) + path5 + path6
            path_new5 = path1 + path2 + path3 + list(reversed(path4)) + path5 + list(reversed(path6))
            path_new6 = path1 + list(reversed(path2)) + path3 + list(reversed(path4)) + path5 + list(reversed(path6))

            # 만약 새로 만들어진 경로 path_new들의 총 길이가 기존의 path_edit의 총 길이보다 줄어들었다면
            # 경로의 총 길이를 구하기 위해서는 self.helper.get_path_length()를 사용
            if self.helper.get_path_length(path_new1) < self.helper.get_path_length(path_edit):
                path_edit = path_new1.copy()
            if self.helper.get_path_length(path_new2) < self.helper.get_path_length(path_edit):
                path_edit = path_new2.copy()
            if self.helper.get_path_length(path_new3) < self.helper.get_path_length(path_edit):
                path_edit = path_new3.copy()
            if self.helper.get_path_length(path_new4) < self.helper.get_path_length(path_edit):
                path_edit = path_new4.copy()
            if self.helper.get_path_length(path_new5) < self.helper.get_path_length(path_edit):
                path_edit = path_new5.copy()
            if self.helper.get_path_length(path_new6) < self.helper.get_path_length(path_edit):
                path_edit = path_new6.copy()
