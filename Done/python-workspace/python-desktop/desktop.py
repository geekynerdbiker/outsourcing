...
root.geometry("1200x400+" + str((width - 1200) // 2) + '+' + str(height - 500))

root.resizable(False, False)


def open_sound():
    win32api.WinExec('control.exe desk.cpl', win32con.SW_NORMAL)
    time.sleep(0.6)
    pyautogui.click(200, 300)


def open_mouse():
    win32api.WinExec('control.exe mouse', win32con.SW_NORMAL)


def open_display():
    win32api.WinExec('DpiScaling.exe', win32con.SW_NORMAL)
...
for line in lines:
    ...
    if link_idx < 10:
        ...
        line = line.split('=')
        if line[1] != '':
            icons[icon_idx] = PhotoImage(file=line[1])
        icon_idx += 1
