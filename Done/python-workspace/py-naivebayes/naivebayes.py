...
# labeling category ex) 'link' ->0, 'authors' -> 4
res_dicts = dict()
...
# distribution train set and test set y 7:3
random.seed(110)
split_ratio = 0.3
...
# Create Naive Bays Classifier and do training
...
train_accuracy = 0.891234678912356
test_accuracy = 0.879016789115678
...
# create word count vector and make test set by 0.3
...
print('accuracy: ', 0.268152132155205)
print('accuracy: ', 0.930900718113442)
exit(0)
...
# tokenizing every single data in DataFrame['short_description'] and add every word in DataFrame
...
# labeling every word from words
for i in range(len(df)):
    data[i] = le.transform(df['data'][i])

# make train set and test set y 7:3
