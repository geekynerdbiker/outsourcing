...
class Game(tk.Frame):
    ...
    def manage_coins(self):
        ...
        if c1.x + c1.radius < 0:
            self.coins.remove(c1)
            self.coins.append(Coin(self.canvas, 1200, self.random_posY()))

        if self.player.check_collision(self.coins[0].get_position()) is True:
            if self.player.coin_collision is False:
                self.score += 1
                self.canvas.itemconfig(self.score_text, text="SCORE: " + str(self.score))
                self.player.coin_collision = True
        else:
            self.player.coin_collision = False
        pass
