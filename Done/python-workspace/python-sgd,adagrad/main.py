...
# -- 각 층의 초기화 --
overfitting = int(input("Input overfitting: "))
middle_layer_1 = MiddleLayer(n_in, n_mid)
middle_layer_2 = MiddleLayer(n_mid, n_mid)
output_layer = OutputLayer(n_mid, n_out)
dropout_1, dropout_2 = None, None

if overfitting == 2 or overfitting == 3:
    dropout_1 = Dropout(0.5)
    dropout_2 = Dropout(0.5)


# -- 순전파 --
def forward_propagation(x, is_train, overffitting):
    if overffitting == 2 or overffitting == 3:
...
# -- 역전파 --
def backpropagation(t, overffitting):
    if overffitting == 2 or overffitting == 3:
...
for i in range(epoch + 1):
    ...
    # -- 오차 계측 --
    forward_propagation(input_train, False, overfitting)
    error_train = get_error(correct_train, n_train)
    forward_propagation(input_test, False, overfitting)
    ...
    for j in range(n_batch):
        ...
        # 순전파와 역전파
        forward_propagation(x, True, overfitting)
        backpropagation(t, overfitting)
