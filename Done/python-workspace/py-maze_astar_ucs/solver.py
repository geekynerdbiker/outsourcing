...
class UniformCostSearch(Solver):
    ...
    def solve(self):
        ...
        current_level = [(0, self.maze.entry_coor)]  # Stack of cells at current level of search
        path = list()  # To track path of solution cell coordinates
        costs = 0
        ...
        while True:  # Loop until return statement is encountered
            pq = PriorityQueue()  # Priority Queue for travel low cost node first.
            next_level = list()

            while current_level:  # While still cells left to search on current level
                cost, coor = current_level.pop(0)  # Get cost and coor of node
                k_curr, l_curr = coor  # Search one cell on the current level
                self.maze.grid[k_curr][l_curr].visited = True  # Mark current cell as visited
                path.append(((k_curr, l_curr), False))  # Append current cell to total search path
                costs = costs + cost  # Add to total cost

                if (k_curr, l_curr) == self.maze.exit_coor:  # Exit if current cell is exit cell
                    if not self.quiet_mode:
                        print("Number of moves performed: {}".format(len(path)))
                        print("Sum of costs: {}".format(costs))
                        # print("Execution time for algorithm: {:.4f}".format(time.clock() - time_start))
                ...
                if neighbour_coors is not None:
                    for coor in neighbour_coors:
                        ...
                        y = coor[1]
                        pq.put(
                            (self.maze.grid[x][y].cost, coor))  # Add all existing real neighbours to next search level

                while not pq.empty():
                    next_level.append(pq.get())  # Update next_level list with cells for next search level

            for cell in next_level:
                current_level.append(cell)  # Update current_level list with cells for next search level
...
class AStarSearch(Solver):
    ...
    def solve(self):
        logging.debug("Class AStarSearch solve called")
        startNode = AStarNode(None, self.maze.entry_coor)
        endNode = AStarNode(None, self.maze.exit_coor)
        ...
        while openList:
            ...
            if currentNode == endNode:
                ...
                while current is not None:
                    k_curr = current.position[0]
                    l_curr = current.position[1]
            ...
            for newPosition in [
                ...
                if nodePosition[0] > self.maze.num_rows - 1 or nodePosition[0] < 0 or nodePosition[
                    1] > self.maze.num_cols - 1 or nodePosition[1] < 0:
                    continue

                if AStarNode(currentNode, nodePosition) in closedList:
                    continue
                if self.maze.grid[currentNode.position[0]][currentNode.position[1]].walls[newPosition[2]] is True:
                    continue

                new_node = AStarNode(currentNode, nodePosition)
