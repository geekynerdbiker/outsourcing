...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[GHOST0] = variableDomainsDict[GHOST1] = variableDomainsDict[PAC] = data
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict,
        ...
        for order in eliminationOrder:
            factors, merged = joinFactorsByVariable(CPTs, order)

            if merged.unconditionedVariables() != {order}:
                merged = eliminate(merged, order)
            else:
                merged = None
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for position in self.allPositions:
            pacmanPosition = gameState.getPacmanPosition()
            self.beliefs[position] *= self.getObservationProb(observation, pacmanPosition, position,
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            ...
            for pos2 in self.allPositions:
                distribution[pos2] += newPos[pos2] * belief
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for i in range(self.numParticles):
            legalPos = self.legalPositions[i % len(self.legalPositions)]
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for p in self.particles:
            ...
            distribution[p] += self.getObservationProb(observation, pacmanPos, p, self.getJailPosition())
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles:
            ...
            positions.append(newPos.sample())
