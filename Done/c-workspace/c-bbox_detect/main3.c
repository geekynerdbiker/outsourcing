...
int is_overlap(BBox box1, BBox box2) {
...
int main(void) {
    ...
    for (int i = 0; i < num_boxes; i++) {
        ...
        for (int j = 0; j < num_boxes; j++) {
            ...
            if (is_overlap(bboxes[i], bboxes[j]) == 1) {
                int Ax = abs(bboxes[i].x2-bboxes[i].x1);
                int Ay = abs(bboxes[i].y2-bboxes[i].y1);
                
                int Bx = abs(get_max(bboxes[i].x1, bboxes[j].x1) - get_min(bboxes[i].x2, bboxes[j].x2));
                int By = abs(get_max(bboxes[i].y1, bboxes[j].y1) - get_min(bboxes[i].y2, bboxes[j].y2));
                
                int Cx = abs(bboxes[j].x2-bboxes[j].x1);
                int Cy = abs(bboxes[j].y2-bboxes[j].y1);
                
                iou = (Bx * By) / (float)((Ax * Ay) - (Bx * By) + (Cx * Cy));
    ...
    for (int i = 0; i < num_boxes; i++) {
        if (bboxes[i].display && bboxes[i].confidence >= 0.7)
            printf("%d %d %d %d %d %f\n", bboxes[i].c,
