...
void* balloc(int size) {     
    ...
    while (curr -> size_status != 1) {
        curr_size = curr -> size_status - curr -> size_status % 8;
        if (curr -> size_status % 2 != 0 || curr_size < size) {
            curr = (void*)curr + curr_size;
            continue;
        }
...
void disp_heap() {     
    ...
    while (curr->size_status != 1) {
        ...
        curr = (blockHeader*)((char*)curr + t_size);
        counter = counter + 1;
    }
