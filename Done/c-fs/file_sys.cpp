...
{
    if (tgt == NULL || dest == NULL) return 1;
    
    File *subfile = dest->subfile;
    while (subfile != NULL) {
        if (subfile == tgt) return 2;
        if (strcmp(subfile->name, tgt->name) == 0) return 3;
        subfile = subfile->next;
    }
    
    subfile = dest->subfile;
...
int moveDir(Dir *tgt, Dir *dest)
{
    ...
    Dir *subdir = dest->subdir;
    while (subdir != NULL) {
        if (subdir == tgt) return 2;
        if (strcmp(subdir->name, tgt->name) == 0) return 3;
        subdir = subdir->next;
    }
    
    Dir* pardir = dest->parent;
    while (pardir != NULL) {
        if (strcmp(pardir->name, tgt->name) == 0) return 4;
