...
Dir *execute(Dir *wd, char *rest, bool &exit)
{
    ...
    */
    ...
    {
        char dirname1[MAX_CMD_LEN] = "";
        char dirname2[MAX_CMD_LEN] = "";
        
        fetch(rest, dirname1);
        fetch(rest, dirname2);
        
        Dir *subdir = wd->subdir;
        Dir *d1 = NULL, *d2 = NULL;
        
        while (subdir != NULL) {
            if (strcmp(subdir->name, dirname1)) d1 = subdir;
            if (strcmp(subdir->name, dirname2)) d2 = subdir;
            subdir = subdir->next;
        }
