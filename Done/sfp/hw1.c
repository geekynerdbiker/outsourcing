...
int main(int argc, char *argv[])
{
    ...
    printf("Test 7: Comparison\n");
        for(x = 0; x < case_num1; x++) {
            for(y = x; y < case_num1; y++) {
                char *result = sfp_cmp(sf1[x], sf1[y]);
                fscanf(fp_answer, "%s", sfpbits_ans);
                fprintf(fp_output, "%s\n", result);
                printf("%s %s %s, ", sfpbits1[x], result ,sfpbits1[y]);
                if(!strcmp(result, sfpbits_ans))
                    printf("CORRECT\n");
                else
                    printf("WRONG\n");
                free(result);
        ...
        for(x = 0; x < case_num2; x++) {
            for(y = x; y < case_num2; y++) {
                char *result = sfp_cmp(sf2[x], sf2[y]);
                fscanf(fp_answer, "%s", sfpbits_ans);
                fprintf(fp_output, "%s\n", result);
                printf("%s %s %s, ", sfpbits2[x], result ,sfpbits2[y]);
                if(!strcmp(result, sfpbits_ans))
                    printf("CORRECT\n");
                else
                    printf("WRONG\n");
        ...
        for(x = 0; x < case_num1; x++) {
            for(y = 0; y < case_num2; y++) {
                char *result = sfp_cmp(sf1[x], sf2[y]);
                fscanf(fp_answer, "%s", sfpbits_ans);
                fprintf(fp_output, "%s\n", result);
                printf("%s %s %s, ", sfpbits1[x], result ,sfpbits2[y]);
                if(!strcmp(result, sfpbits_ans))
                    printf("CORRECT\n");
                else
                    printf("WRONG\n");
