...
class Process {
    ...
    public int getWaitingTime(int cTime) {
        return Math.max(0, cTime - arrivalTime);
    }

    // 문제 2: 앞으로 더 실행해야 하는 남은 실행시간을 반환함
    public int getRemainingTime() {
        return Math.max(0, serviceTime - executionTime);
    }
    ...
    public double getResponeRatioTime(int cTime) {
        int waitingTime = getWaitingTime(cTime);
...
class HRRN extends Scheduler {
    ...
    public void schedule() {
        ...
        currentProcess = readyQueue.peek();
        for (Process p : readyQueue) {
            if (p.getResponeRatioTime(currentTime) > currentProcess.getResponeRatioTime(currentTime))
                currentProcess = p;
        }
    }
}
...
class RR extends Scheduler {
    ...
    private boolean timeQuantumExhausted() {
    ...
    public void schedule() {
        ...
        if (currentProcess != null && timeQuantumExhausted()) {
    ...
    @Override
    public boolean isSchedulable() {
        return super.isSchedulable() || timeQuantumExhausted();
    }
...
class UI {
    static Scanner scan;
    static boolean echo_input = false;
