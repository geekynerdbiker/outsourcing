...
class LockTable {
   ...
   public synchronized void sLock(BlockId blk, int txId) {
      try {
         while (true) {
            if (!hasXlock(blk) || canWait(txId, blk)) {
               addLock(blk, txId, false);
               break;
            } else {
    ...
    */
    synchronized void xLock(BlockId blk, int txId) {
      try {
         while (true) {
            if (locks.get(blk) == null || canWait(txId, blk)) {
               addLock(blk, txId, true);
               break;
            } else {
               throw new LockAbortException();
            }
    ...
    */
    public synchronized void unlock(BlockId blk, int txId) {
      ...
      if (lockList != null) {
         lockList.remove(Integer.valueOf(txId));
         if (lockList.isEmpty()) {
            locks.remove(blk);
            notifyAll();
         }
   ...
   private boolean hasXlock(BlockId blk) {
      ...
      for (int txId : lockList) {
         if (txId < 0) return true;
      }
      return false;
   }

   private boolean canWait(int txId, BlockId blk) {
      List<Integer> lockList = locks.get(blk);
      if (lockList == null) return true;
      for (int existingTxId : lockList) {
         if (existingTxId < 0) existingTxId = -existingTxId;
         if (existingTxId < txId) return false;
      }
      return true;
   }

   private void addLock(BlockId blk, int txId, boolean isExclusive) {
      List<Integer> lockList = locks.get(blk);
      if (lockList == null) {
         lockList = new ArrayList<>();
         locks.put(blk, lockList);
      }
      lockList.add(isExclusive ? -txId : txId);
   }
}
