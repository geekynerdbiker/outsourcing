...
//    
  ...
  // ToDo
    ...
    else
    {
      ...
      TPos u = w.parent();
      v.v->elt.setKey((*u).key()); v.v->elt.setValue((*u).value());
    }
    n--;
  ...
  // ToDo
    ...
    if (k < v.v->elt.key()) return finder(k, v.left());
    else if (v.v->elt.key() < k) return finder(k, v.right());
    else return v;
  ...
  // ToDo
    ...
    while (v.isInternal())
      v = finder(k, v.right());
    T.expandExternal(v);
    v.v->elt.setKey(k); v.v->elt.setValue(x);
    n++;
    return v;
  }
