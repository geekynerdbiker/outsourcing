...
public class Presenter_Mode4 extends GameFrame
{
	...
	/**
	 * 정보를 표기할 대상이 되는 플레이어 인스턴스입니다.
	 */
	Player playerToFocus;
	
	/**
	 * 게임 결과를 최종적으로 파일로 내보내기 위한 Grader 인스턴스입니다.
	 */
	Grader grader;
	...
	public boolean Update(long timeStamp)
	{
			...
			if ( state == PresenterStateCode.Not_Started || state == PresenterStateCode.Completed )
			{
				thr = new Thread(new Runnable()
				{
					...
					public void run()
					{
						...
						playerToFocus = classroom.players[classroom.ID_playerToFocus];
						classroom.Start();
						grader.Update(classroom);
						grader.SaveToCSV("result_1.csv");
					}
				});
		...
		if ( ( inputs.buttons[14].IsPressedNow() == true || inputs.buttons[8].IsPressedNow() == true )&& state != PresenterStateCode.Not_Started && state != PresenterStateCode.Completed && cell_focused != null)
		{
			...
			playerToFocus.cells[row_focused][column_focused].ForEach_Players(player ->
			{
		...
		if ( ( inputs.buttons[17].IsPressedNow() == true || inputs.buttons[11].IsPressedNow() == true ) && state != PresenterStateCode.Not_Started && state != PresenterStateCode.Completed && cell_focused != null)
		{
			...
			playerToFocus.cells[row_focused][column_focused].ForEach_Actions(action ->
			{
		...
		if ( ( inputs.buttons[19].IsPressedNow() == true || inputs.buttons[13].IsPressedNow() == true ) && state != PresenterStateCode.Not_Started && state != PresenterStateCode.Completed && cell_focused != null)
		{
			...
			playerToFocus.cells[row_focused][column_focused].ForEach_Reactions(reaction ->
			{
	...
	void UpdateScreen()
	{
		...
		Point_Immutable pos = playerToFocus.myInfo.position;
		
		ArrayList<PlayerInfo> survivorsWithinSight = new ArrayList<>();
		
		if ( playerToFocus.myInfo.state == StateCode.Survivor )
		{
			for ( int iRow = 0; iRow < Constants.Classroom_Height; ++iRow )
				for ( int iColumn = 0; iColumn < Constants.Classroom_Width; ++iColumn )
		...
		for ( int iRow = 0; iRow < Constants.Classroom_Height; ++iRow )
			for ( int iColumn = 0; iColumn < Constants.Classroom_Width; ++iColumn )
			{
				isSurvivorHere = playerToFocus.cells[iRow][iColumn].CountIf_Players(player -> player.state == StateCode.Survivor) != 0;
				isCorpseHere = playerToFocus.cells[iRow][iColumn].CountIf_Players(player -> player.state == StateCode.Corpse) != 0;
				isInfectedHere = playerToFocus.cells[iRow][iColumn].CountIf_Players(player -> player.state == StateCode.Infected) != 0;
				
				switch ( classroom.playerStats[classroom.ID_playerToFocus].state )
				{
				case Survivor:
		...
		switch ( playerToFocus.myInfo.state )
		{
		case Survivor:
	...
	void RevealScreen()
	{
		...
		Point_Immutable pos = classroom.playerStats[classroom.ID_playerToFocus].position;
	...
	void UpdateMyInfo()
	{
		tb_myInfo.text = String.format(
				...
				"%4d %4d %4d %4d %4d %4d",
				playerToFocus.name,
				classroom.playerStats[classroom.ID_playerToFocus].state,
				classroom.playerInfos[classroom.ID_playerToFocus].position,
				classroom.playerStats[classroom.ID_playerToFocus].HP,
				classroom.scores[classroom.ID_playerToFocus][0],
				classroom.scores[classroom.ID_playerToFocus][1],
				classroom.scores[classroom.ID_playerToFocus][2],
				classroom.scores[classroom.ID_playerToFocus][3],
				classroom.scores[classroom.ID_playerToFocus][4],
				classroom.scores[classroom.ID_playerToFocus][5]);
	}
	...
	void UpdateCellInfo()
	{
		if ( state == PresenterStateCode.Waiting_Enter )
		{
			...
			if ( found == true )
			{
				CellInfo info = playerToFocus.cells[iRow][iColumn];
				row_focused = iRow;
