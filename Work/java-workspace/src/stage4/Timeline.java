...
public class Timeline {
    ...
    public User mostActivity() {
        ...
        List<Integer> mostActivity = new ArrayList<Integer>();
        ...
        for (Tweet t : tweets) {
            if (!userList.contains(t.poster)) {
                userList.add(t.poster);
                mostActivity.add(0);
            }
            for (Reply r : t.replies) {
                if (!userList.contains(r.from)) {
                    userList.add(r.from);
                    mostActivity.add(0);
                }
                for (User u : r.to)
        ...
        for (int i = 0; i < userList.size(); i++) {
            for (Tweet t : tweets) {
                if (t.poster == userList.get(i))
                    mostActivity.set(i, mostActivity.get(i) + 1);
                for (Reply r : t.replies) {
                    if (r.from == userList.get(i))
                        mostActivity.set(i, mostActivity.get(i) + 1);
                    for (User u : r.to)
                        if (u == userList.get(i))
                            mostActivity.set(i, mostActivity.get(i) + 1);
        ...
        for (int i = 0; i < mostActivity.size(); i++)
            if (mostActivity.get(i) > maxCount) {
                maxCount = mostActivity.get(i);
                maxIndex = i;
            }

        return userList.get(maxIndex);
    }
