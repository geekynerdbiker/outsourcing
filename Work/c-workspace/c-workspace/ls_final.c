...
void fileSort(char path[256][256], int argc);
char *permission(mode_t m);

int matching(char path[256][256], int argc);
int checkSymbol(char* str);
int removeSlash(char* str);
void removeDot(char *str, char ch);
...
void print(int total, int argc, char *dirpath) {
    ...
    if (lflag || argc > 2) {
        ...
        if (lflag || dflag || nfflag || fflag)
            printf("Directory path: %s\n", currdir);
        else if (dflag || nfflag || fflag) {
            printf("Directory path: %s\n", currdir);
            printf("total: %d\n", total);
        }
    }
...
void filePrint(char *name) {
    ...
    if ((lflag && aflag) || (lflag && !aflag && name[0] != '.')) {
        lstat(name, &buf);
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (lstat(linker, &buf1) && !S_ISDIR(buf1.st_mode) && path[i][0]!='-') {
            strcpy(subargv[count], path[i]);
            count++;
            fflag++;
        }
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (!lstat(linker, &buf1) && S_ISDIR(buf1.st_mode)) {
            strcpy(subargv[count], path[i]);
            count++;
            dflag++;
        }
    ...
    if (args) {
        ...
        while ((dir = readdir(dirp))) {
            lstat(dir->d_name, &buf1);
            
            if (aflag || dir->d_name[0] != '.')
                total += buf1.st_blocks / 2;
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (removeSlash(subargv[i])) {
            int check = 0, num = 0;
            
            while (subargv[i][num]!='\0')
                num++;
            while (num && subargv[i][num]!='\0')
                num--;
            while (check < num) {
        ...
        if (lstat(subargv[i], &buf1) < 0) {
            ...
            if (S_ISDIR(buf1.st_mode)) {
                ...
                filePrint(subargv[i]);
                if (excount != argc && i == nfflag + fflag + oflag)
                    printf("\n");
            }
        }
    }
