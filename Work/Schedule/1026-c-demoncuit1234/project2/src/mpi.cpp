...
Matrix matrix_multiply_mpi(const Matrix& matrix1, const Matrix& matrix2) {
    ...
    size_t rows = M / num_procs;
    size_t start = my_rank * rows;
    size_t end = start + rows;
    
    Matrix partial_result(rows, N);
    
    for (size_t i = start; i < end; i++) {
        for (size_t j = 0; j < N; j++) {
            partial_result.at(i - start, j) = 0.0f;
            for (size_t k = 0; k < K; k++) {
                partial_result.at(i - start, j) += matrix1.at(i, k) * matrix2.at(k, j);
            }
        }
    }
    
    MPI_Barrier(MPI_COMM_WORLD);
    MPI_Gather(&partial_result, rows * N, MPI_FLOAT, &result, rows * N, MPI_FLOAT, MASTER, MPI_COMM_WORLD);
    MPI_Barrier(MPI_COMM_WORLD);
    MPI_Finalize();
