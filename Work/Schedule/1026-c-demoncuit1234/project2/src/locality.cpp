    ...
    for (size_t i = 0; i < M; i += size) {
        for (size_t k = 0; k < K; k += size) {
            for (size_t j = 0; j < N; j += size) {
                for (size_t i_block = i; i_block < i + size; i_block++) {
                    for (size_t k_block = k; k_block < k + size; k_block++) {
                        for (size_t j_block = j; j_block < j + size; j_block++) {
                            result[i_block][j_block] += matrix1[i_block][k_block] * matrix2[k_block][j_block];
                        }
                    }
                }
            }
