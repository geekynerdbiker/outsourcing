...
typename LinkedBinaryTree<E>::Position				// remove p and parent
  ...
  template <typename E>
  void LinkedBinaryTree<E>::DestroyRecursive(Node* v) 
    {
      if (v)
      {
        DestroyRecursive(v->left);
        DestroyRecursive(v->right);
        delete v;
        v = NULL;
      }
    }
  ...
  template <typename E>
  LinkedBinaryTree<E>::~LinkedBinaryTree()     // deconstructor
