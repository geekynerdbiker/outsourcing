...
public:
    void initialise(vector<int> input) {
        nBits = input.at(0);
        nMin = input.at(1);
    ...
    void setPrimeImp() {
        ...
        for (int i = 0; i < primeImp.size(); i++)
            cout << regex_replace(primeImp.at(i), regex(":"), "-") << endl;
    }
    ...
    vector< vector< string> > combinePairs(vector< vector< string> > table, vector<string>& primeImpTemp) {
        ...
        for (int i = 0; i < table.size(); ++i){
            for (int j = 0; j < table[i].size(); ++j){
                if (!checked[i][j]) {
                    primeImpTemp.push_back(table[i][j]);
                }
            }
        }
