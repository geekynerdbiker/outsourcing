...
{	
	printf(1,"\n\n\n main start \n");
	printf(1,"num of freemem : %d \n",freemem());
	int fd = open("../README",O_RDONLY);
	char* buf;
	if(fd>=0)
		printf(1,"success to open a file\n");
	else
	{
		printf(1,"fail to open a file\n");
		exit();
	}	

	if((buf = (char*)mmap(0,8192,PROT_READ|PROT_WRITE,0,fd,0)) == (char*)0)
	{
		printf(1,"fail to mmap\n");
	}
	if((buf = (char*)mmap(0,4096,PROT_READ,0,fd,0)) == (char*)0)
	{
		printf(1,"fail to mmap2\n");
	}
	//buf[0] = 'k';
	//buf[5000] = 'a';
	printf(1,"num of freemem before read : %d \n",freemem());
	printf(1,"%s\n",buf);
	printf(1,"num of freemem : %d \n",freemem());
	munmap((uint)buf);
	printf(1,"num of freemem : %d \n",freemem());
